...
为增强dom节点的使用，增加了指令功能，指令用于模板串中，指令以“x-”开头，作为属性放置于元素头中，见下面代码的`x-repeat`，目前NoDom支持指令:module,model,repeat,class,if,else,show,field,route,router，详情见[指令](#指令)。
...
### 事件(NEvent)
事件和元素的事件相对应，以"e-"开头，覆盖html标准事件类型，模板中事件定义不能带参数，NoDom会自动传递约定的参数，共四个，依次为：
序号| 参数 | 类型
-|-|-
...
```
#### 模板(Template)
模板在模块中用`template()`进行声明，参数为props，props为从父模块(如果该模块为子模块)对应元素传递的属性(attribute)，改写上例：
...
#### 指令简写方式
Nodom提供了指令简写方式，可以通过自定义元素方式简写指令。将在后续每个指令单独讲解。  
自定义元素经过编译之后默认为`div元素`，若想使用其它元素，可通过tag属性指定，下面是repeat指令简写的一个示例：
```html
<!-- 未指定tag，默认为div -->
<for cond={{rows}}>
	<span>{{name}}</span>
...
```
**简写方式**  
repeat指令可以用`for`元素进行简写，指令值用cond属性进行配置，改写上面的模板如下：
```js
class Main extends Module{
	template(){
		return `
...
recur指令可以用`recur`元素进行简写，指令值用cond属性进行配置，从上面的例子可以看到`recur`元素的用法。
...
使用module元素或module类名两种方式进行简写，改写上例的模板如下：
```js
class Main extends Module{
	<!--声明 MmoduleA-->
	modules=[ModuleA]
...
#### route 指令
route将当前dom设定路由节点，点击dom将跳进行路由跳转。使用方式如下：
```html
<a x-route='path'>跳转到path</a>
```
可使用`route`元素进行替代，route指令的值由path代替。默认元素为`a`,如果修改，则设置`tag`属性。
改写上例代码如下：
...
2. `Nodom.registModule`方法注册
    `Nodom.registModule`方法可以给待注册模块设置**别名**，在模板代码中使用模块时，既可以使用模块类名作为元素名引入，也可以使用注册的别名作为元素名引入。
...
显示为`my name is :noomi`;

#### 匿名插槽

如果子模块内`slot`元素无`name`属性，则模块(如下面的`modulea`)元素内的元素会替换子模块的`slot`元素。
...
class Main extends Module{
    ...
    template(){
        return `
			<div>
  				<modulea>
					<!--下面的p和button元素会替换ModuleA的slot元素-->
					<p>我是父模块的P元素</p>
					<button>我是父模块</button>
				 </modulea>
			</div>
...
在实际使用中，可能需要多个插槽，就需要使用命名插槽，通过插槽的`name`属性设置插槽名字。命名插槽就是给插槽定义插槽名，传入的元素需要与插槽名一致才可发生替换。
...
// 根模块  modulea元素内的slot元素内容作为待插入的内容
class Main extends Module{
	modules=[ModuleA];
    template(){
        return `
		<div>
 		 	<modulea>
				<slot name='title'>
					<!--替换ModuleA<slot name='title'>元素-->
					<button>我是父模块的title</button>
				<slot>
				<slot name='footer'>
					<!--替换ModuleA<slot name='name'>元素-->
...
每个`Model`存有一个模块列表，当`Model`内部的数据变化时，会引起该`Model`的模块列表中所有模块的渲染。一个`Model`的模块列表中默认只有初始化该`Model`的模块，当存在slot或模块传值为**对象**时，将会导致Model绑定到多个模块，当然也可以通过`ModelManager的bindToModule`方法绑定。
...
class Main extends Module{
    template(){
        return `
			<div>
				<button e-click='change'>change</button>
				<button e-click='watchActive'>watch</button>
...
​	Nodom对CSS提供额外的支持。在模板中使用`<style></style>` 元素中直接写入CSS样式，示例代码如下：
...
在模板代码中的 `<style></style>` 元素中通过表达式调用函数返回CSS样式代码串，示例代码如下：
...
在模板代码中的 `<style></style>` 元素中通过@import url('CSS url路径')引入CSS样式文件，示例代码如下：
...
The framework now supports a Vue 3 style composition entry without changing the
existing template, directive or event syntax.
...
class Counter extends Module {
    template() {
        return `
            <div>
                ...
                <p>{{doubleCount}}</p>
                <p>{{profile.name}}</p>
                <button e-click="increase">increase</button>
            </div>
        `;
    }
    ...
    setup() {
        ...
        const profile = useReactive({ name: "nodom" });
...
- `template()` syntax stays the same, so `{{count}}`, `{{profile.name}}` and
  `e-click="increase"` do not need `.value`.
- `setup()` return values are exposed to the current module automatically.
